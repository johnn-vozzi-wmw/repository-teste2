...
public class ListMetasPorGrupoProduto1Form extends BaseCrudListForm {

	private static final String INITIAL_VALUE_TOTALIZADORES = "999999999,9999999";
    ...
    public ListMetasPorGrupoProduto1Form() throws SQLException {
        super(Messages.MENU_OPCAO_METAS_GRUPO_PRODUTO);
        singleClickOn = true;
        setBaseCrudCadForm(new CadMetasPorGrupoProduto1Form(1, false));
        cbRepresentante = new RepresentanteSupervComboBox(RepresentanteSupervComboBox.DefaultItemType_ALL);
        cbPeriodo = new PeriodoComboBox(PeriodoComboBox.PERIODOMETASGRUPOPRODUTOPRODUTO1);
		...
		if (totalizadoresMetaVisible) {
			lbTotalMeta = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[0] + INITIAL_VALUE_TOTALIZADORES);
			lbTotalRealizado = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[1] + INITIAL_VALUE_TOTALIZADORES, RIGHT);
		}
		if (totalizadoresQtUnidadeVisible) {
			lbTotalQtUnidadeMeta = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[3] + INITIAL_VALUE_TOTALIZADORES);
			lbTotalQtUnidadeRealizado = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[4] + INITIAL_VALUE_TOTALIZADORES, RIGHT);
		}
		if (totalizadoresQtCaixaVisible) {
			lbTotalQtCaixaMeta = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[6] + INITIAL_VALUE_TOTALIZADORES);
			lbTotalQtCaixaRealizado = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[7] + INITIAL_VALUE_TOTALIZADORES, RIGHT);
		}
		if (totalizadoresQtMixClienteVisible) {
			lbTotalMixClienteMeta = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[9] + INITIAL_VALUE_TOTALIZADORES);
			lbTotalMixClienteRealizado = new LabelTotalizador(Messages.LABEL_TOTAL_ABREVIADO + colunasGrid[10] + INITIAL_VALUE_TOTALIZADORES, RIGHT);
		}
	}
    ...
    protected Vector getDomainList() throws java.sql.SQLException {
		...
		metasPorGrupoProduto1Filter.dsPeriodo = cbPeriodo.getValue();
		metasPorGrupoProduto1Filter.cdGrupoProduto1Like = edFiltro.getValue();
		if (ValueUtil.isNotEmpty(metasPorGrupoProduto1Filter.cdGrupoProduto1Like)) {
			return MetasPorGrupoProdutoService.getInstance().findMetasByGrupoProduto(metasPorGrupoProduto1Filter, 1);
		}
    	return MetasPorGrupoProdutoService.getInstance().findAllMetasByGrupoProduto(metasPorGrupoProduto1Filter, 1);
    }
    ...
    protected String[] getItem(Object domain) throws SQLException {
        ...
        if (LavenderePdaConfig.isMostraDetalhesAdicionaisRelMetasPorGrupoProduto1Ligado()) {
        	...
        	item.addElement(metasPorGrupoProduto1.cdGrupoProduto1);
        	if (ValueUtil.isEmpty(metasPorGrupoProduto1.dsGrupoProduto1) && !LavenderePdaConfig.usaRelMetasGrupoProdIndepDoCadDeGrupoProd) {
        		item.addElement(GrupoProduto1Service.getInstance().getDsGrupoProduto(metasPorGrupoProduto1.cdGrupoProduto1));
        	} else {
        		if (metasPorGrupoProduto1.dsGrupoProduto1 == null || metasPorGrupoProduto1.dsGrupoProduto1.equalsIgnoreCase(metasPorGrupoProduto1.cdGrupoProduto1)) {
        			item.addElement("[" + metasPorGrupoProduto1.cdGrupoProduto1 + "]");
        		} else {
        			item.addElement(metasPorGrupoProduto1.dsGrupoProduto1 + " [" + metasPorGrupoProduto1.cdGrupoProduto1 + "]");
        		}
        	}
        	...
        	if (ValueUtil.isEmpty(metasPorGrupoProduto1.dsGrupoProduto1) && !LavenderePdaConfig.usaRelMetasGrupoProdIndepDoCadDeGrupoProd) {
        		...
        		if (metasPorGrupoProduto1.dsGrupoProduto1 == null || metasPorGrupoProduto1.dsGrupoProduto1.equalsIgnoreCase(metasPorGrupoProduto1.cdGrupoProduto1)) {
        			...
        			item.addElement(metasPorGrupoProduto1.dsGrupoProduto1 + (LavenderePdaConfig.usaDescricaoCodigoNaVisualizacaoEntidades == true ? "  [" + metasPorGrupoProduto1.cdGrupoProduto1 + "]":""));
        		}
        	}
        	item.addElement(StringUtil.getStringValueToInterface(metasPorGrupoProduto1.vlMeta));
        	vlTtMeta += metasPorGrupoProduto1.vlMeta;
	...
	protected void onFormStart() throws SQLException {
		...
		if (SessionLavenderePda.isUsuarioSupervisor()) {
			...
			UiUtil.add(this, new LabelName(Messages.REPRESENTANTE_NOME_ENTIDADE), new LabelValue(SessionLavenderePda.getRepresentante().toString()), getLeft(), getTop());
		}
		UiUtil.add(this, new LabelName(Messages.METAS_META), cbPeriodo, getLeft(), getNextY());
        ...
        if (LavenderePdaConfig.isMostraDetalhesAdicionaisRelMetasPorGrupoProduto1Ligado()) {
        	...
        	GridColDefinition[] gridColDefiniton = {
    			...
    			new GridColDefinition(GrupoProduto1Service.getInstance().getLabelGrupoProduto1(), ww * 6, LEFT),
    ...
    public void loadCbPeriodo() throws SQLException {
    	...
    	Vector vector = MetasPorGrupoProdutoService.getInstance().findAllMetasVigentes(metasPorGrupoProdutoFilter);
    	if (ValueUtil.isEmpty(vector)) {
    		cbPeriodo.setSelectedIndex(0);
    	}
    	int nuDiasOld = 400;
    	String dsPeriodo = null;
    	for (int i = 0; i < vector.size(); i++) {
    		MetasPorGrupoProduto metasPorGrupoProduto = (MetasPorGrupoProduto) vector.items[i];
    		int nuDiasNew = DateUtil.getDaysBetween(metasPorGrupoProduto.dtFinal, metasPorGrupoProduto.dtInicial);
    		if (nuDiasNew < nuDiasOld) {
    			nuDiasOld = nuDiasNew;
    			dsPeriodo = metasPorGrupoProduto.dsPeriodo;
    		}
		}
    	if (ValueUtil.isNotEmpty(dsPeriodo)) {
    		cbPeriodo.setValue(dsPeriodo);
    	}
    }
    ...
    private GridColDefinition[] gridColDefinitionDetalhesAdicionais(int ww) {
    	...
    	gridColDefinitonTemp.addElement(new GridColDefinition(GrupoProduto1Service.getInstance().getLabelGrupoProduto1(), ww * 4, LEFT));
    	for (int i = 0; i < colunasGrid.length; i++) {
    ...
    public void listAndAddItensOnGrid() throws SQLException {
    	flListInicialized = true;
    	list();
    }
  ...
  //@Override
    public void detalhesClick() throws SQLException {
		...
		metasPorGrupoProduto.dsPeriodo = cbPeriodo.getValue();
		metasPorGrupoProduto.cdGrupoProduto1 = cdGrupoProdutoSelected;
		metasPorGrupoProduto.cdGrupoProduto2 = MetasPorGrupoProduto.CDGRUPOPRODUTO_NULO;
		...
		if (ValueUtil.isEmpty(getCdRepresentante())) {
			Vector list = MetasPorGrupoProdutoService.getInstance().findAllMetasByGrupoProduto(metasPorGrupoProduto, 1);
			if (list.size() > 0) {
		...
		//--
		if (LavenderePdaConfig.isNivelGrupoProd2()) {
			ListMetasPorGrupoProduto2Form listMetasPorGrupoProduto2Form = new ListMetasPorGrupoProduto2Form(cdGrupoProdutoSelected, cbPeriodo.getValue());
			show(listMetasPorGrupoProduto2Form);
		} else {
			ListMetasPorGrupoProduto4Form listMetasPorGrupoProduto4Form = new ListMetasPorGrupoProduto4Form(cdGrupoProdutoSelected, MetasPorGrupoProduto.CDGRUPOPRODUTO_NULO, MetasPorGrupoProduto.CDGRUPOPRODUTO_NULO, cbPeriodo.getValue());
