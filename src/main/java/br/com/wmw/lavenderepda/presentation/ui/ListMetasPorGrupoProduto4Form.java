...
public class ListMetasPorGrupoProduto4Form extends BaseCrudListForm {
    ...
    public ListMetasPorGrupoProduto4Form(String cdGrupoProduto1, String cdGrupoProduto2, String cdGrupoProduto3, String dsPeriodo) throws SQLException {
        super(Messages.MENU_OPCAO_METAS_GRUPO_PRODUTO);
        singleClickOn = true;
        int nuNivel = LavenderePdaConfig.usaFiltroGrupoProduto == 0 ? 4 : LavenderePdaConfig.usaFiltroGrupoProduto; 
        setBaseCrudCadForm(new CadMetasPorGrupoProduto1Form(nuNivel, true));
    ...
    protected Vector getDomainList() throws java.sql.SQLException {
		...
		metasPorGrupoProduto1Filter.cdGrupoProduto3 = cdGrupoProduto3;
		return MetasPorGrupoProdutoService.getInstance().findAllMetasByGrupoProduto(metasPorGrupoProduto1Filter, 4);
    }
    ...
    protected String[] getItem(Object domain) throws SQLException {
        ...
        if (LavenderePdaConfig.isMostraDetalhesAdicionaisRelMetasPorGrupoProduto1Ligado()) {
        	...
        	item.addElement(metasPorGrupoProduto1.cdProduto);
        	item.addElement(ProdutoService.getInstance().getDescriptionWithId(metasPorGrupoProduto1.cdProduto));
        	...
        	String[] item = {
        			StringUtil.getStringValue(metasPorGrupoProduto1.cdProduto),
        			StringUtil.getStringValue(ProdutoService.getInstance().getDescriptionWithId(metasPorGrupoProduto1.cdProduto)),
        			StringUtil.getStringValueToInterface(metasPorGrupoProduto1.vlMeta),
        			StringUtil.getStringValueToInterface(metasPorGrupoProduto1.vlRealizado),
        			StringUtil.getStringValueToInterface(metasPorGrupoProduto1.getPctRealizadoMeta()),
        			StringUtil.getStringValue(DateUtil.formatDateDDMMYYYY(metasPorGrupoProduto1.dtInicial)),
        			StringUtil.getStringValue(DateUtil.formatDateDDMMYYYY(metasPorGrupoProduto1.dtFinal))};
        	vlTtMeta += metasPorGrupoProduto1.vlMeta;
        	vlTtRealizado += metasPorGrupoProduto1.vlRealizado;
        	return item;
        }
    ...
    protected void onFormStart() throws SQLException {
        ...
        if (LavenderePdaConfig.isNivelGrupoProd3()) {
        	UiUtil.add(this, new LabelName(GrupoProduto3Service.getInstance().getLabelGrupoProduto3()), new LabelValue(GrupoProduto3Service.getInstance().getDsGrupoProduto(cdGrupoProduto1, cdGrupoProduto2, cdGrupoProduto3)), getLeft(), getNextY(), PREFERRED);
        }
        //--
        if (LavenderePdaConfig.isMostraDetalhesAdicionaisRelMetasPorGrupoProduto1Ligado()) {
    ...
    public void detalhesClick() throws SQLException {
    	...
    	metasPorGrupoProduto.cdGrupoProduto3 = cdGrupoProduto3;
    	metasPorGrupoProduto.cdProduto = cdGrupoProdutoSelected;
    	if (ValueUtil.isEmpty(getCdRepresentante())) {
			Vector list = MetasPorGrupoProdutoService.getInstance().findAllMetasByGrupoProduto(metasPorGrupoProduto, 4);
			if (list.size() > 0) {
    ...
    private GridColDefinition[] gridColDefinitionDetalhesAdicionais(int ww) {
    	...
    	gridColDefinitonTemp.addElement(new GridColDefinition(Messages.PRODUTO_NOME_ENTIDADE, ww * 4, LEFT));
    	for (int i = 0; i < colunasGrid.length; i++) {
